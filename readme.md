...
1. The sales data file is read while handling different file encodings.  
2. The raw data is cleaned and invalid records are removed.  
3. The user is shown available regions and can optionally filter the data.  
4. Sales analytics such as revenue, top products, customers and daily trends are calculated.  
5. Product information is fetched from the DummyJSON API.  
6. Sales data is enriched using the API product details.  
7. Enriched data is saved to `data/enriched_sales_data.txt`.  
8. A professional sales report is generated in `output/sales_report.txt`.  
